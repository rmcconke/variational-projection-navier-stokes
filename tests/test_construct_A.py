...
class TestConstructA:
    ...
    def test_shape(self):
        ...
        A = construct_A(grid, dx=0.1, dy=0.1)
        assert A.shape == (P * Q, 2 * P * Q)
    ...
    def test_side_boundaries_have_3_entries(self):
        ...
        for boundary in [grid['left_b'], grid['right_b'], 
                         grid['bottom_b'], grid['top_b']]:
            for m in boundary:
                nnz = np.count_nonzero(A[m, :])
    ...
    def test_divergence_free_field_gives_zero(self):
        ...
        U_dot = np.zeros(2 * P * Q)
        for m in range(P * Q):
            col = m % P
            row = m // P
            x = (col + 1) * dx  # +1 because grid points are interior
            y = (row + 1) * dy
    ...
    def test_coefficients_scale_with_grid(self):
        ...
        A1 = construct_A(grid, dx1, dy1)
        A2 = construct_A(grid, dx2, dy2)
    ...
    def test_small_grid_manual_check(self):
        ...
        row4_nnz = np.nonzero(A[4, :])[0]
        assert set(row4_nnz) == {3, 6, 10, 15}

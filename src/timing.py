...
# Timing create_grid_indices


grid = create_grid_indices(P, Q)
stats = timer(lambda: create_grid_indices(P, Q), repeat=10)
print(f"create_grid_indices: {stats['mean']:.4f}s ± {stats['std']:.4f}s")

## Timing construct_A
A = construct_A(grid, dx, dy)
stats = timer(lambda: construct_A(grid, dx, dy), repeat=10)
print(f"construct_A: {stats['mean']:.4f}s ± {stats['std']:.4f}s")

## Timing compute_M_neg_sqrt
M_neg_sqrt = compute_M_neg_sqrt(P, Q, rho, dx, dy)
stats = timer(lambda: compute_M_neg_sqrt(P, Q, rho, dx, dy), repeat=10)
...
# Timing construct_C
stats = timer(lambda: construct_C(grid, dx, dy, U, rho, nu, U_lid), repeat=10)
